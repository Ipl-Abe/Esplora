...
//ロボット移動速度切り替え用
unsigned long speed, old_speed,turn_speed = 16,turn_dfspeed = 16;
...
void setup () {
  ...
  //表示用の関数を２つ作成
  showEsplora("1",70,30,5);
  showEsplora2(speed_s,70,100,2);
...
void agz_controlRobot(){
  ...
  if((val[4] == LOW) && (old_val[4] == HIGH)){
    controller[Lmode] = 1;
    controller[Rmode] = 2;
    controller[Lspeed] = turn_dfspeed;
    controller[Rspeed] = turn_dfspeed;
...
void loop () {
  ...
  //add  2016/05/17
  //右スティックを倒してボタンを押し、速度を変える
  else if(Esplora.readJoystickX() < -511 && Esplora.readJoystickY() <300 && Esplora.readJoystickY() > -300)
    {
      agz_speedChange();  
    }
